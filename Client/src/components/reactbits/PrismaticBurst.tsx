...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    (container as HTMLDivElement).addEventListener('pointermove', onPointer as any, { passive: true });
    ...
    return () => {
      ...
      try {
        if (meshRef.current) (meshRef.current as any).remove?.();
      } catch (e) {
        /* ignore dispose errors */
      }
      try {
        if (triRef.current) (triRef.current as any).remove?.();
      } catch (e) {
        /* ignore dispose errors */
      }
      try {
        if (programRef.current) (programRef.current as any).remove?.();
      } catch (e) {
        /* ignore dispose errors */
      }
      try {
