...
import { getPriorities, setPriority, markFinished, updateStatus, reorderPriorities } from '../../services/api';
...
export default function Top5Page() {
    ...
    useEffect(() => { localStorage.setItem('fomo_vista_top5', vista); }, [vista]);
    ...
    const onDrop = (dropIndex: number) => {
        ...
        if (user) {
            reorderPriorities(user.id, newOrder.map((p, i) => ({ gameId: p.gameId, priorityOrder: i + 1 })));
        }
    };
    ...
    const numClass = (n: number) => `priority-num-${n <= 3 ? n : n <= 4 ? '4' : '5'}`;
                ...
                <div className="top5-status-right">
                    <div className="top5-dots-container">
                        {Array.from({ length: 5 }).map((_, i) => (
                            <div key={i} className={i < slotCount ? 'top5-dot-active' : 'top5-dot-inactive'} />
                        ))}
                    </div>
                    <button
                    ...
                    {priorities.map((juego: any, index: number) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`priority-num ${numClass(index + 1)}`}>{index + 1}</div>
                    ...
                    {Array.from({ length: emptySlots }).map((_, i) => {
                        ...
                        return (
                            <div key={`empty-${i}`} className="priority-slot empty">
                                <div className={`priority-num ${numClass(slotNum)}`}>{slotNum}</div>
                    ...
                    {priorities.map((juego: any, index: number) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`top5-card-badge ${numClass(index + 1)}`}>{index + 1}</div>
                    ...
                    {Array.from({ length: emptySlots }).map((_, i) => {
                        ...
                        return (
                            <div key={`empty-${i}`} className="top5-card top5-card-empty">
                                <div className={`top5-card-badge ${numClass(slotNum)}`}>{slotNum}</div>
